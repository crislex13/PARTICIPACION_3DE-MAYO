...
public class VentaDAOimpl extends ConexionDB implements VentaDAO {
    ...
    public void insert(Venta venta) throws Exception {
        try {
            this.conectar();
            PreparedStatement ps = (PreparedStatement) this.conn.prepareStatement("insert into ventas (producto_id,cliente_id,fecha) values (?,?,?)");
    ...
    public void delete(int id) throws Exception {
        try {
            this.conectar();
            PreparedStatement ps = (PreparedStatement) this.conn.prepareStatement("delete from ventas where id=?");
            ps.setInt(1, id);
            ps.executeUpdate();
        } catch (Exception e) {
            throw e;
    ...
    public Venta getById(int id) throws Exception {
        ...
        try {
            this.conectar();
            PreparedStatement ps = (PreparedStatement) this.conn.prepareStatement( "select from ventas where id=?");
            ps.setInt(1, id);
